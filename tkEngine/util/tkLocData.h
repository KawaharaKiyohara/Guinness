...
namespace tkEngine{
	...
	public:
		...
		using QueryLocObjectFunction = std::function<void(const SObjectData&)>;
		CLocData()
		{
		}
		~CLocData()
		...
		*/
		...
		/*!
		*@brief	配置されているオブジェクトに対してクエリを行う。
		*/
		void QueryLocObject(QueryLocObjectFunction function)
		{
			for (int i = 0; i < GetNumObject(); i++) {
				function(m_objectData.at(i));
			}
		}
	private:
